...
- **공백 신호는 걷어내지 않는다 — 잡별 루프 대신 단발 확인으로 커버한다.** 표의
  ...
  공백 동안의 수단은 **이벤트 수신 시·턴 시작 시의 단발 조회 1회**다
  (`herdr agent list`·허브 `/v1/jobs/orphaned`·`panewire jobs` 같은, 잡 수에
  비례하지 않는 fleet 호출 — 잡 하나를 찝을 때도 반복하지 않고 그 시점의
  목록 조회로 대체한다) — 잡별 루프가 아니므로 상한 0을 깨지 않고, 위 동시
  상한 안에 센다. #80·#66 배포로 push 가 생기면 이 임시 수단은 걷어낸다.
