...
| 변경 표면 | T floor |
|---|---|
| 브로커·주문·live·deploy·안전가드 경로 | T3 |
| auto_trader 비거래 경로 | T2 |
| 도구 repo(scopefuel·agent-skills 등) | T1 |
| 로컬 스크립트·설정 ≤30줄 | T0(floor 없음) |

- **하향은 거부한다.** 상류가 T1을 제안해도 표면이 브로커면 T3다(ROB-1189가 이 규칙으로
  막혔을 사례). **상향은 자유**(판단이 필요하면 올린다).
...
1. **미측정 우선 배정**: 해당 급에 `추정` 표기 프로필이 있고 그 풀에 쿼타 여유가 있으면 **우선 배정**을 고려한다. 특히 풀이 `[WASTE] 리셋 전 소진 권장` 이면 측정 비용이 사실상 0이다.
   - 🔴 **T1·T2 비거래 작업 한정**: **T3·주문·배포·안전가드 표면에서는 절대 하지 마라.** 미검증 프로필로 되돌리기 비싼 작업을 수행하는 것은 측정 이득보다 위험이 크다.
...
ROB-1150 실주문 4건. **5건 중 5건이 명세 단계 결함이고 실행 단계 결함은 0건이다** —
