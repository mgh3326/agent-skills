...
🔴 **이미 push 된 브랜치의 base 갱신 = rebase 가 아니라 merge / update-branch (2026-09-03)**

- **push 전** 로컬 브랜치는 rebase 로 정리해도 된다.
- 🔴 **push 된 뒤 base 를 따라잡을 때는 `git merge origin/main`(또는 GitHub *Update branch*)를
  쓴다.** force-push 는 **이력을 파괴**하고, 리뷰 코멘트·검증 라운드가 가리키던 SHA 를 날린다.
  (적대검증 보고서가 `<head SHA>` 를 인용하는 이 프로그램에서는 특히 비싸다.)
- **base 전진은 머지 상신의 필수 확인 항목이다**: `git merge-base --is-ancestor <main> <head>`
  가 참이어야 한다. 🔴 **"브랜치 CI green" 은 "main green" 을 뜻하지 않는다** —
  2026-09-03 실사고: #2007 이 #2019 머지 **이전** base 에서 CI 를 통과한 채 머지돼
  **main 이 RED** 가 됐다(테스트가 #2019 의 신규 스코프를 몰랐다). 상신 브리프에
  **"마지막 CI 이후 base 전진 여부"** 를 필드로 요구하라.

🔴 **`git status` clean 요구와 보고서가 충돌한다 (2026-09-03 실사고)**

브리프에 *"종료 시 `git status --short` 빈 출력"* 을 넣으면서 보고서를 worktree 안에 쓰게 하면
🔴 **워커가 그 조건을 맞추려고 보고서를 지운다.** 판정은 화면에만 남고 **산출물이 사라진다.**

- **보고서 경로는 항상 worktree 밖**(`~/work/herdr-inbox/jobs/<job_id>/`)으로 주고,
  🔴 브리프에 **"이 경로는 worktree 밖이라 `git status` 에 영향이 없다 — 지우지 마라"** 를
  한 줄 덧붙여라. 경로만 줘도 워커가 worktree 안에 쓰는 일이 생긴다.
- 회수 시 **지정 경로에 없으면 pane 을 닫기 전에 먼저 회수**하라. pane 이 살아 있으면
  *"다시 써라(재검증 아님)"* 로 복구된다 — **닫으면 영구 소실**이다.
- 🔴 같은 날 3가지 변형이 다 나왔다: ① 검증자가 **다른 job 디렉토리**에 씀(bl48v·fndv)
  ② 설계자가 worktree 에 쓰고 **git exclude** 로 우회(pklogb) ③ 검증자가 **삭제**(pklogv).
  ①②는 회수 가능하지만 ③은 pane 을 닫았으면 잃는다.

🔴 **관측성 기능 = post-deploy witness 를 완료 조건에 넣어라 (2026-09-03, 동형 2건째)**

로깅·계측·기록 기능(픽 로그·감사 원장·텔레메트리 등)을 발주할 때는 **"배포 완료"를 완료로
치지 마라.** 배포는 됐는데 **아무도 그 경로를 부르지 않아 0행**인 실패가 반복됐다.

- **완료 조건에 반드시 넣을 항목**: 🔴 *"배포 후 24h 내 **실제 실행 흔적 1행 이상**을
  실측으로 제시"* (테이블 count · 로그 라인 · 스팬 중 **관측 가능한 것 하나**를 지정).
  없으면 그 기능은 **미완**이지 완료가 아니다.
- 🔴 **"코드가 배선돼 있다"는 증거가 아니다.** 배선·env·유닛테스트가 전부 초록이어도
  **호출 지점이 없으면 0행**이다. 호출 지점은 대개 **이 레포 밖**(운영자 프롬프트·
  스케줄러·외부 세션)에 있어서 코드 리뷰로는 절대 안 잡힌다.
- 브리프에 **"누가 이 경로를 부르는가"를 파일·프롬프트 이름으로 명시**하게 하고,
  그 호출 지점이 레포 밖이면 **그 변경까지 발주 범위에 포함**하거나 **별건으로 명시 인계**하라.

**근거(동형 2건)**: ROB-1283 기각 코호트 — 어휘·스키마·핸들러 배포 완료, 라이브 프롬프트가
그 도구 이름을 대지 않아 **66일간 0행**. §140 screener_pick_log — env·코드·writer 전부 정상,
`discover_buy_candidates_fanout` 을 **아무 세션도 호출하지 않아 전 기간 0행**(도구를 수동으로
1회 부르자 즉시 120행 기록되어 파이프라인 정상이 증명됐다). 둘 다 **수리 지점이 레포 밖**이었다.
