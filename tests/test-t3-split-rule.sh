...
FILE_FORBIDDEN = [
    ...
    ("verdict-inherit-after-change",
     ...
     r"갈음|대신|대체|유지)|(?:squash\w*|amend\w*|rebas\w*|rewrit\w*|revis\w*|force[- ]?push|patch|updat\w*|cherry[- ]?pick\w*|picked|grafted|ported|transplanted|reapplied|restored|체리|스쿼시|어?멘드|개정|리베이스|패치|수정|새\s*head|follow[- ]?up|subsequent|후속)[^\n]{0,25}?(?:inherit\w*|substitut\w*|replac\w*|reuse|re-?use|carr\w*|retain\w*|transfer\w*|grandfather\w*|rides?|borrow\w*|stands?\s+on|rests?\s+on|leans?\s+on|relies?\s+on|preserv\w*|saves?|safeguard\w*|conserv\w*|승계|계승|갈음|대신|대체|유지|그대로)[^\n]{0,25}?(?:verification|verdict|review|approval|sign[- ]?(?:ed?[-\s]*)?off|PASS|accept\w*|green\s*light|receipt|token|records?|acceptance|outcomes?|results?|go/no[- ]?go|go-no-go|decisions?|determinations?|calls?|judgments?|clearance|clearances|attestation|attestations|certif\w*|ratif\w*|tests?|testing|시험|테스트|승인|판정|검증|통과|결과|효력)[^\n]{0,30}?(?:head|revision|commit|"
     r"commits?|sha|version|replacement|successor|descendant|derived|downstream|tip|커밋|버전|리비전))|(?:new|revised|updated|patched|rebased|squashed|amended|cherry[- ]?picked|grafted|ported|descendant|derived|downstream|successor|follow[- ]?up|subsequent|새|새로운|수정된|리베이스된)[^\n]{0,5}?(?:head|revision|commit|sha|version|커밋|버전|리비전)[^\n]{0,25}?(?:inherit\w*|substitut\w*|replac\w*|reuse|re-?use|carr\w*|retain\w*|transfer\w*|grandfather\w*|rides?|keep\w*|holds?|stands?\s+on|borrow\w*|adopt\w*|rests?\s+on|leans?\s+on|relies?\s+on|승계|계승|갈음|대신|대체|유지|그대로)[^\n]{0,15}?(?:under|from|of|의|으로|에)?\s*(?:the\s+|its\s+|their\s+|this\s+|that\s+)?(?:previous|prior|old|earlier|parent|ancestor|predecessor|grandparent|tester|reviewer|auditor|examiner|inspector|existing|"
    ...
    ("core-class-lower-t",
     ...
     r"clerk|caretaker|custodian|janitor|temp\w*|casual|part[-\s]?time|rotating|ad[-\s]?hoc|temporary|contract|support|vendor|freelance|crew|external|outside|third[- ]?party|consult\w*|helper|assistant|bargain|frugal|thrifty|discount|budget|low[- ]paid|cut[- ]?rate|staffing|agency|offshore|nearshore|pooled|in[-\s]?house|operations|ops\b|back[- ]?office|support[- ]?desk|billing|사무|심부름|서류|보조|부수|잡무|단순|청소|싼|저렴|하급|초급|주변부|가장자리|저비용|인력)[^\n]{0,15}?(?:주변|배정|실행|처리|돌리|맡기|떼|취급|간주|본다|여긴다|핫픽스|작업|분류|넘긴다|보낸다|업무|워커|레인|"
    ...
    ("periph-core-no-reclassify",
     ...
     r"|output|read[-\s]?only|theming|surface|layout|screen|icon|spacing|styling|styled|decor\w*|ornament\w*|aesthetic|cli\b|command[-\s]?line|front[- ]?end|web|read|query|command|cmd)(?:[-\s](?:ticket|item|work|task|job|change|patch|issue|label|category|classif\w*|tier|grade|lane|bucket|bracket|bin|작업|항목))?)|(?:원래|기존|처음|당초)\s*(?:분류|등급|명칭|라벨|범주|클래스|tier|grade|category|label)[^\n]{0,10}?(?:유지|그대로|둔다|남는다|동일)|(?:핵심|core|안전|가드)[^\n]{0,20}?(?:slip\w*|sneak\w*|leak\w*|creep\w*|touch\w*|end\w*|land\w*|enter\w*)[^\n]{0,15}?(?:into|in|to|onto|"
