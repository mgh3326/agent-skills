...
FILE_FORBIDDEN = [
    ...
    ("integration-one-pass-en",
     r"(?i)(?:final\s+)?(?:integration|merge(?:r|d|s)?|merging|combin\w+|consolidat\w*|assembl\w*|결합|병합|머지|합침|통합)[^\n]{0,45}?(?:(?:only\s+one|\bone\b|single|a\s+single|just\s+one|one-?shot|single-?shot|1(?!\s*차))[^\n]{0,15}?(?:pass|round|look|check|review|verification|glance|peek|once-?over|runthrough|walkthrough|sign[- ]?(?:ed?[-\s]*)?off|signoff|inspection|scrutiny|assessment|audit|examin(?:ation|e)|perusal|scan|survey|sweep|read[- ]through|dry[- ]run|test\b|probe|double[- ]check|skim|browse|approval)|(?:run|look|review|verify|check|use|read|skim|scan|browse)[ -]?once|once\s+only)|(?:only\s+one|"
     r"\bone\b|single|a\s+single|just\s+one|one-?shot|single-?shot|1(?!\s*차))[^\n]{0,15}?(?:pass|round|look|check|review|verification|glance|peek|once-?over|runthrough|walkthrough|sign[- ]?(?:ed?[-\s]*)?off|signoff|inspection|scrutiny|assessment|audit|examin(?:ation|e)|perusal|scan|survey|sweep|read[- ]through|dry[- ]run|test\b|probe|double[- ]check|skim|browse|approval)[^\n]{0,45}?(?:final\s+)?(?:integration|merge(?:r|d|s)?|merging|combin\w+|consolidat\w*|assembl\w*|결합|병합|머지|합침|통합)"),
    ("integration-initial-pass-suffices",
    r"(?i)(?:initial|first|single|one|lone|sole|opening)[^\n]{0,15}?(?:sign[- ]?(?:ed?[-\s]*)?off|signoff|pass|round|review|inspection|check|audit|examin\w+|glance|look|attestation|attestations|certif\w*|clearance|clearances|verdict|assessment|evaluation)[^\n]{0,25}?(?:alone|by itself|on its own|suffices|is enough|conclusive|decisive|adequate|sufficient|enough|all|needed|required|충분|족하|혼자)[^\n]{0,25}?(?:final\s+)?(?:integration|merge(?:r|d|s)?|merging|combin\w+|amend\w*|chang\w*|revision\w*|patch\w*|rebas\w*|rewrit\w*|updat\w*|version|결합|병합|머지|합침|통합|수정|개정)|(?:final\s+)?(?:integration|merge(?:r|d|s)?|merging|combin\w+|consolidat\w*|assembl\w*|결합|병합|머지|합침|통합)[^\n]{0,45}?(?:initial|first|single|one|lone|sole|"
    r"opening)[^\n]{0,15}?(?:sign[- ]?(?:ed?[-\s]*)?off|pass|round|review|inspection|check|audit|examin\w+|glance|look)[^\n]{0,20}?(?:alone|by itself|on its own|suffices|is enough|conclusive|decisive|adequate|sufficient|enough|all|needed|required|충분|족하|혼자)|(?:initial|first|one|single)[^\n]{0,15}?(?:pass|round|inspection|review|check|audit|examin\w+|sign[- ]?offs?)[^\n]{0,30}?(?:later|subsequent|further|additional|extra|more|second|follow-?up|나중|후속|추가|뒤)[^\n]{0,15}?(?:passes?|rounds?|inspections?|reviews?|checks?|audits?|examin\w+|ones?|패스|"
    r"라운드|검토|검사)?[^\n]{0,30}?(?:at[^\n]{0,15}?(?:discretion|choice)|discretionary|optional|unnecessary|not needed|waivable|skippable|dispensable|unneeded|up\s+to\s+(?:the\s+)?\w+|owner'?s?\s+(?:call|choice|discretion)|선택|생략|불필요|재량|자유)|(?:lone|"
     r"single|one|sole|initial|first|only|mere|lonesome)\s+[^\n]{0,20}?(?:merge\s+|integration\s+|final\s+)*(?:certif\w*|sign[- ]?(?:ed?[-\s]*)?off|approval|verdict|PASS|review|audit|check|inspection|examination|look|glance|assessment|attestation|clearance|ratif\w*|인증|승인|검증)[^\n]{0,20}?(?:settles?|decides?|concludes?|determines?|resolves?|finalizes?|closes?|completes?|ratif\w*|disposes?|handles?|covers?"
     r"|suffices?|answers?|정한다|결정|마무리|완결)[^\n]{0,20}?(?:integration|merge|merging|결합|병합|머지|통합)(?:[^\n]{0,15}?(?:conclusively|decisively|finally|definitively|sufficiently|alone|once|for\s+good|for\s+all|permanently|결정적|최종|완결))?|(?:final\s+)?(?:integration|merge(?:r|d|s)?|merging|combin\w+|consolidat\w*|assembl\w*|결합|병합|머지|합침|통합)[^\n]{0,45}?(?:is\s+|are\s+|be\s+|gets?\s+|was\s+|been\s+)?(?:settled|decided|determined|resolved|closed|done|finished|completed|concluded|disposed|discharged|ended|finalized|정해|결정|마무리|종결)[^\n]{0,15}?(?:by|with|after|on|via|through|으로|에서)\s+(?:the\s+|a\s+|an\s+|one\s+|its\s+)?(?:first|initial|single|sole|lone|opening|1st|earliest|primary|첫|처음|"
    r"최초)[^\n]{0,15}?(?:sign[- ]?(?:ed?[-\s]*)?off|pass|round|review|inspection|check|audit|examin\w+|glance|look|assessment|verdict|evaluation|scrutiny|acceptance|adoption|receipt|attestation|approv\w*|confirm\w*|검토|검사|심사|평가)|(?:final\s+)?(?:integration|merge(?:r|d|s)?|merging|combin\w+|consolidat\w*|assembl\w*|결합|병합|머지|합침|통합)[^\n]{0,30}?(?:is|are|be|gets?|was)?[^\n]{0,10}?(?:accept\w*|approv\w*|adopt\w*|ratif\w*|clos\w*|settl\w*|done|finish\w*|complet\w*|conclud\w*|wrap\w*|finali[sz]\w*|resolv\w*|sign\w*|승인|완료|종결)[^\n]{0,15}?(?:once|when|after|upon|as\s+soon\s+as|the\s+moment)[^\n]{0,30}?(?:the\s+|an?\s+|its\s+|that\s+)?[^\n]{0,20}?(?:reviewer|tester|auditor|approver|verifier|checker|inspector|검토자|승인자|테스터|감사)[^\n]{0,15}?(?:sign\w*|approv\w*|verdict|pass|accept\w*|confirm\w*|attest\w*|certif\w*|green\s*light|ok|승인|확인|통과|판정|서명)|(?:최종|"
     r"마지막)\s*(?:통합|합침|머지|병합|결합)[^\n]{0,10}?(?:은|는|을|를)?[^\n]{0,10}?(?:최초|처음|첫|단일|한\s*번|한번)[^\n]{0,10}?(?:검토자|검토|승인자|테스터|확인자|reviewer|tester|auditor)[^\n]{0,5}?(?:의)?[^\n]{0,10}?(?:확인|승인|판정|통과|검증|서명)[^\n]{0,5}?(?:으로|로)?[^\n]{0,10}?(?:종결|종료|마무리|완료|처리|끝)"),
    ("integration-cap-numeric",
    r"(?i)(?:(?:final\s+)?(?:integration|merge(?:r|d|s)?|merging|combin\w+|consolidat\w*|assembl\w*|결합|병합|머지|합침|통합)[^\n]{0,45}?(?:(?:cap\w*|limit\w*|ceil\w*|restrict\w*|confine\w*|ceiling|up\s+to|consume|permit\w*|allow\w*|use|uses|employ\w*|상한|캡|제한|둔다|정한|고정|끝|마친|마무리|종결|끝낸|끝나|종료|충분|허용|가능|enough|suffic\w*|stop|halt|end|finish|cease|conclud\w*)[^\n]{0,20}?(?:[01245-9]|one|two|zero|four|five|six|seven|eight|nine|ten|eleven|twelve|dozen|1[0-9]|many|several|multiple|numerous|unlimited|unbounded|한|두|네|다섯|여섯|일곱|여덟|아홉|열|열한|열두|십|다수|여러|무제한|단일|single|couple(?:\s+of)?|pair(?:\s+of)?|thirteen|fourteen|fifteen|sixteen|seventeen|eighteen|nineteen|twenty|thirty|forty|fifty|sixty|seventy|eighty|ninety|hundred|thousand|score|scores|"
     r"\d{2,}|스물|서른|마흔|쉰|예순|일흔|여든|아흔|백|천|first|second|third|fourth|1st|2nd|3rd|4th|첫|두\s*번째|세\s*번째)\s*(?:passes?|rounds?|회|차례|패스|라운드|번|looks?|reviews?|checks?|sign[- ]?offs?|"
    r"cycles?|audits?|inspections?|examin\w+|evaluations?|assessments?|verifications?)|(?:[01245-9]|one|two|zero|four|five|six|seven|eight|nine|ten|eleven|twelve|dozen|1[0-9]|many|several|multiple|numerous|unlimited|unbounded|한|두|네|다섯|여섯|일곱|여덟|아홉|열|열한|열두|십|다수|여러|무제한|단일|single|couple(?:\s+of)?|pair(?:\s+of)?|thirteen|fourteen|fifteen|sixteen|seventeen|eighteen|nineteen|twenty|thirty|forty|fifty|sixty|seventy|eighty|ninety|hundred|thousand|score|scores|\d{2,}|스물|서른|마흔|쉰|예순|일흔|여든|아흔|백|천|first|second|third|fourth|1st|2nd|3rd|4th|첫|두\s*번째|세\s*번째)\s*(?:passes?|rounds?|회|차례|패스|라운드|번|looks?|reviews?|checks?|sign[- ]?offs?|cycles?|audits?|inspections?|examin\w+|evaluations?|assessments?|"
    r"verifications?)[^\n]{0,20}?(?:cap\w*|limit\w*|ceil\w*|restrict\w*|confine\w*|ceiling|up\s+to|consume|permit\w*|allow\w*|use|uses|employ\w*|상한|캡|제한|둔다|정한|고정|끝|마친|마무리|종결|끝낸|끝나|종료|충분|허용|가능|enough|suffic\w*))|(?:[01245-9]|one|two|zero|four|five|six|seven|eight|nine|"
    r"한|두|네|다섯|여섯|일곱|여덟|아홉|단일|single|couple(?:\s+of)?|pair(?:\s+of)?)\s*(?:permitted|allowed|available|remaining|left|남은|허용|가능)?\s*(?:passes?|rounds?|회|차례|패스|라운드|번|looks?|reviews?|checks?|sign[- ]?offs?|cycles?|audits?|inspections?|examin\w+|verifications?)(?!\s*(?:한정|상한|캡|제한|규칙)[은이는란])[^\n]{0,20}?(?:for|of|on|per|at|에|으로)?\s*(?:the\s+)?(?:final\s+)?(?:integration|merge(?:r|d|s)?|merging|combin\w+|consolidat\w*|assembl\w*|결합|병합|머지|합침|통합)|(?:unbounded|boundless|uncapped|limitless|unlimited|unrestricted|unconstrained|infinite|endless|no\s+(?:upper\s+|hard\s+|firm\s+)?(?:bound|limit|cap|ceiling|restriction|constraint)|without\s+(?:any\s+|an?\s+)?(?:(?:upper|hard|firm|round|review|pass|fixed|set|stated|declared|maximum|max|formal|specified|finite)\s+)?(?:bounds?|limits?|caps?|ceilings?|restrictions?|constraints?|ceil))[^\n]{0,20}?(?:passes?|rounds?"
     r"|회|차례|패스|라운드|번|looks?|reviews?|checks?|sign[- ]?offs?|cycles?|audits?|inspections?|examin\w+|evaluations?|assessments?|verifications?)[^\n]{0,25}?(?:final\s+)?(?:integration|merge(?:r|d|s)?|merging|combin\w+|consolidat\w*|assembl\w*|결합|병합|머지|합침|통합)|(?:final\s+|closeout|closing|wrap[- ]?up)?(?:integration|merge(?:r|d|s)?|merging|combin\w+|assembl\w*|closeout|closing|wrap[- ]?up|결합|병합|머지|합침|통합|종료|마무리)[^\n]{0,45}?(?:(?:unbounded|boundless|uncapped|limitless|unlimited|unrestricted|unconstrained|infinite|endless|continu\w+|go(?:es)?\s+on|carry(?:ing)?\s+on|repeat\w*|drag\w*|proceed\w*)[^\n]{0,25}?(?:on\s+|endlessly|indefinitely|forever|for\s+good|permanently|without\s+(?:an?\s+)?end|끝없이|무한(?:으로)?)?[^\n]{0,10}?(?:passes?|rounds?|회|차례|패스|라운드|번|looks?|reviews?|checks?|sign[- ]?offs?|cycles?|audits?|inspections?|examin\w+|evaluations?|assessments?|verifications?)|"
    r"(?:without|sans|with\s+no|no)\s+(?:any\s+|an\s+)?(?:upper\s+|hard\s+|firm\s+)?(?:bounds?|limit|cap|ceiling|restriction|constraint)|(?:may|might|can|could)\s+(?:continue|proceed|run|go|extend|stretch|drag|repeat|revisit)[^\n]{0,15}?(?:(?:without|sans|with\s+no|no)\s+(?:any\s+|an\s+)?(?:upper\s+|hard\s+|firm\s+)?(?:bounds?|limit|cap|ceiling|restriction|constraint)|indefinitely|interminably|perpetually|in\s+perpetuity|forever|endlessly|infinitely|as\s+long\s+as\s+(?:needed|required|necessary|it\s+takes)|until\s+(?:done|finished|satisfied|complete)|무한히|무제한|끝없이)|(?:is|are|be|remains?|stays?|seems?|appears?|counts?\s+as|becomes?|turns?|goes|gets?|stands?|남는다|유지|두고|둔다)\s+(?:unbounded|boundless|uncapped|limitless|unlimited|unrestricted|unconstrained|infinite|endless|무제한|무한|상한\s*없)|(?:stop|halt|end|finish|close|conclude|complete|wrap|terminate|cease|cap|limit|restrict|cut|curtail|trim|그만|종료|멈추|"
    r"제한)\w*[^\n]{0,15}?(?:after|at|beyond|past|over|within|to|in|on|by|후에|에|에서)?\s*(?:[01245-9]|one|two|zero|four|five|six|seven|eight|nine|ten|eleven|twelve|dozen|score|fifteen|twenty|thirty|한|두|네|다섯|여섯|일곱|여덟|아홉|열|열한|열셋|스무|단일|single|couple(?:\s+of)?|pair(?:\s+of)?)\s*(?:(?:audit|review|verification|inspection|check|test|merge|integration|검증|검토|심사|확인)\s+)*(?:rounds?|passes?|loops?|iterations?|cycles?|attempts?|reviews?|verifications?|inspections?|checks?|audits?|examinations?|evaluations?|assessments?|회|번|차|라운드|회차|검증|검토)|(?:no|not\s+a?ny?|zero|none|lacking|missing|absent|omitted|sans|without|없|미지정)\s+(?:a\s+|an\s+|any\s+)?(?:finite|fixed|set|"
    r"hard|firm|specified|stated|declared|announced|pronounced|published|posted|defined|explicit|enumerated|specific|definite|particular|concrete|maximum|max\b|정해진|고정된|명시된|선언된|최대)\s*(?:(?:review|verification|audit|inspection|check|round|pass|testing|examination|scrutiny|cycle|attempt|merge|integration|검증|검토|심사|확인|회|차례|번)\s+)*(?:quota|caps?|limits?|ceiling|bounds?|maximum|max\b|loops?|iterations?|counts?\b|number|tally|캡|상한|제한|쿼터)))|(?:there\s+(?:is|are)\s+)?(?:no|not\s+a?ny?|zero|none|lacking|missing|absent|sans|without|없)\s+(?:a\s+|an\s+|any\s+)?(?:finite|fixed|set|hard|firm|specified|stated|declared|announced|pronounced|published|posted|defined|explicit|enumerated|specific|definite|particular|concrete|정해진|고정된|"
    r"명시된|선언된)\s*(?:ceiling|cap\w*|limits?|bounds?|maximum|max\b|quota|캡|상한|제한|쿼터)[^\n]{0,45}?(?:integration|merge(?:r|d|s)?|merging|combin\w+|consolidat\w*|assembl\w*|결합|병합|머지|합침|통합|review|verification|검증|검토)"),
    ("verdict-inherit-after-change",
     r"(?i)(?:previous|prior|old|earlier|first|initial|parent|pre[- ]?squash|original|existing|stale|ancestor|predecessor|grandparent|기존|이전|부모|조상|선행)[^\n]{0,25}?(?:sign[- ]?(?:ed?[-\s]*)?off|approval|verdict|review|verification|PASS|accept\w*|green\s*light|go[-\s]?ahead|clearance|clearances|disposition|status|receipt|token|records?|file|log|acceptance|승인|판정|검증|통과|결과|허가|효력)[^\n]{0,25}?(?:substitut\w*|inherit\w*|replac\w*|suffic\w*|enough|stands?|holds?|appl\w*|carr\w*|counts?|serves?|acts?|works?|covers?|governs?|rules?|binds?|controls?|dictates?|grandfather\w*|rides?|transfer\w*|rolls?\s+over|borrow\w*|stands?\s+on|rests?\s+on|leans?\s+on|relies?\s+on|builds?\s+on|draws?\s+on|instead|in\s+(?:its|their|"
     r"the)\s+place|갈음|대신|대체|승계|계승|유효|충분|그대로)[^\n]{0,30}?(?:head|revision|commit|merged|amended|squashed|new|changed|fresh|updated|tip|커밋|변경|수정|머지|병합|리비전|replacement|sha|version|버전|successor|descendant|derived|downstream|tree|graft|result)|(?:squash\w*|amend\w*|rebas\w*|rewrit\w*|revis\w*|force[- ]?push|patch|updat\w*|cherry[- ]?pick\w*|picked|grafted|ported|transplanted|reapplied|restored|rebuilt|remade|recreated|repushed|체리|스쿼시|어?멘드|개정|리베이스|패치|수정|새\s*head|follow[- ]?up|subsequent|후속)[^\n]{0,30}?(?:commits?|head|revision|sha|version|커밋|버전|리비전)[^\n]{0,25}?(?:(?:inherit\w*|substitut\w*|replac\w*|keep\w*|reuse|re-?use|carr\w*|retain\w*|"
     r"hold\w*|stand\w*|suffic\w*|enough|grandfather\w*|rides?|transfer\w*|갈음|대신|대체|승계|계승|유지|충분)[^\n]{0,35}?(?:verification|verdict|review|approval|sign[- ]?(?:ed?[-\s]*)?off|PASS|accept\w*|green\s*light|receipt|token|records?|acceptance|판정|검증|승인|통과|결과|효력)|(?:verification|verdict|review|approval|sign[- ]?(?:ed?[-\s]*)?off|PASS|accept\w*|green\s*light|receipt|token|records?|acceptance|outcomes?|results?|go/no[- ]?go|go-no-go|decisions?|determinations?|calls?|judgments?|clearance|clearances|attestation|attestations|certif\w*|ratif\w*|tests?|testing|시험|테스트|status|state|green\s*status|승인|판정|검증|통과|결과|효력)[^\n]{0,20}?(?:inherit\w*|substitut\w*|replac\w*|carr\w*|retain\w*|transfer\w*|grandfather\w*|rides?|borrow\w*|stands?\s+on|rests?\s+on|leans?\s+on|relies?\s+on|승계|계승|"
     r"갈음|대신|대체|유지)|(?:squash\w*|amend\w*|rebas\w*|rewrit\w*|revis\w*|force[- ]?push|patch|updat\w*|cherry[- ]?pick\w*|picked|grafted|ported|transplanted|reapplied|restored|rebuilt|remade|recreated|repushed|체리|스쿼시|어?멘드|개정|리베이스|패치|수정|새\s*head|follow[- ]?up|subsequent|후속)[^\n]{0,25}?(?:inherit\w*|substitut\w*|replac\w*|reuse|re-?use|carr\w*|retain\w*|transfer\w*|grandfather\w*|rides?|borrow\w*|stands?\s+on|rests?\s+on|leans?\s+on|relies?\s+on|preserv\w*|saves?|safeguard\w*|conserv\w*|승계|계승|갈음|대신|대체|유지|그대로)[^\n]{0,25}?(?:verification|verdict|review|approval|sign[- ]?(?:ed?[-\s]*)?off|PASS|accept\w*|green\s*light|receipt|token|records?|acceptance|outcomes?|results?|go/no[- ]?go|go-no-go|decisions?|determinations?|calls?|judgments?|clearance|clearances|"
     r"attestation|attestations|certif\w*|ratif\w*|tests?|testing|시험|테스트|status|state|green\s*status|승인|판정|검증|통과|결과|효력)[^\n]{0,30}?(?:head|revision|commit|"
     r"commits?|sha|version|replacement|successor|descendant|derived|downstream|tip|커밋|버전|리비전))|(?:new|revised|updated|patched|rebased|squashed|amended|cherry[- ]?picked|grafted|ported|descendant|derived|downstream|successor|rebuilt|remade|recreated|follow[- ]?up|subsequent|패치된|머지된|갱신된|새|새로운|수정된|리베이스된)[^\n]{0,5}?(?:head|revision|commit|sha|version|커밋|버전|리비전)[^\n]{0,25}?(?:inherit\w*|substitut\w*|replac\w*|reuse|re-?use|carr\w*|retain\w*|transfer\w*|grandfather\w*|rides?|keep\w*|holds?|stands?\s+on|borrow\w*|adopt\w*|rests?\s+on|leans?\s+on|relies?\s+on|승계|계승|갈음|대신|대체|유지|그대로)[^\n]{0,15}?(?:under|from|of|의|으로|에)?\s*(?:the\s+|its\s+|their\s+|this\s+|that\s+)?(?:previous|prior|old|earlier|parent|ancestor|predecessor|grandparent|tester|reviewer|auditor|examiner|inspector|existing|"
     r"stale|기존|이전|부모|조상|선행)[^\n]{0,15}?(?:'s\s+)?(?:verification|verdict|review|approval|sign[- ]?(?:ed?[-\s]*)?off|PASS|accept\w*|green\s*light|receipt|token|records?|acceptance|outcomes?|results?|go/no[- ]?go|go-no-go|decisions?|determinations?|calls?|judgments?|clearance|clearances|attestation|attestations|certif\w*|ratif\w*|tests?|testing|시험|테스트|status|state|green\s*status|승인|판정|검증|통과|결과|효력)|(?:squash\w*|amend\w*|rebas\w*|rewrit\w*|revis\w*|force[- ]?push|patch|updat\w*|cherry[- ]?pick\w*|picked|grafted|ported|transplanted|reapplied|restored|rebuilt|remade|recreated|repushed|체리|스쿼시|어?멘드|개정|리베이스|패치|수정|새\s*head|follow[- ]?up|subsequent|후속)[^\n]{0,25}?(?:inherit\w*|substitut\w*|replac\w*|reuse|re-?use|carr\w*|retain\w*|transfer\w*|"
     r"grandfather\w*|rides?|borrow\w*|stands?\s+on|rests?\s+on|leans?\s+on|relies?\s+on|preserv\w*|saves?|safeguard\w*|conserv\w*|승계|계승|갈음|대신|대체|유지|"
     r"그대로)[^\n]{0,25}?(?:verification|verdict|review|approval|sign[- ]?(?:ed?[-\s]*)?off|PASS|accept\w*|green\s*light|receipt|token|records?|acceptance|outcomes?|results?|go/no[- ]?go|go-no-go|decisions?|determinations?|calls?|judgments?|clearance|clearances|attestation|attestations|certif\w*|ratif\w*|tests?|testing|시험|테스트|status|state|green\s*status|승인|판정|검증|통과|결과|효력)[^\n]{0,30}?(?:head|revision|commit|commits?|sha|version|replacement|successor|descendant|derived|downstream|tip|커밋|버전|리비전)|(?:verification|verdict|review|approval|sign[- ]?(?:ed?[-\s]*)?off|PASS|accept\w*|green\s*light|receipt|token|records?|acceptance|outcomes?|results?|decisions?|determinations?|calls?|judgments?|dispositions?|audits?|승인|판정|검증|통과|결과|효력)[^\n]{0,15}?(?:from|of|on|for|at|covering|about|regarding|made\s+(?:on|at)|"
     r"earned\s+(?:on|at)|issued\s+(?:on|at)|granted\s+(?:on|at)|attached\s+to|tied\s+to)[^\n]{0,15}?(?:the\s+|an?\s+|its\s+|that\s+|this\s+)?(?:old|previous|prior|earlier|base|parent|original|ancestor|predecessor|stale|former|past|source|origin|upstream|기존|이전|부모|조상|베이스|원본|선행|소스)[^\n]{0,15}?(?:SHA|sha|head|commit|revision|version|tip|커밋|리비전|버전)[^\n]{0,25}?(?:remains?|stays?|holds?|stands?|still|keeps?|continues?|persists?|is|are|becomes?|goes|counts?\s+as|serves?\s+as|acts?\s+as|works?\s+as|functions?\s+as|유지|유효|갈음|충분)[^\n]{0,15}?(?:binding|authoritative|controlling|governing|decisive|conclusive|definitive|final|sufficient|enough|valid|good|operative|in\s+force|in\s+effect|유효|갈음|충분|"
     r"결정적)|(?:verification|verdict|review|approval|sign[- ]?(?:ed?[-\s]*)?off|PASS|accept\w*|green\s*light|receipt|token|records?|acceptance|outcomes?|results?|decisions?|determinations?|calls?|judgments?|dispositions?|audits?|승인|판정|검증|통과|결과|효력)[^\n]{0,15}?(?:from|of|on|for|at)[^\n]{0,15}?(?:the\s+|an?\s+|its\s+|that\s+|this\s+)?(?:old|previous|prior|earlier|base|parent|original|ancestor|predecessor|stale|former|source|origin|upstream|기존|이전|부모|조상|베이스|원본|선행|소스)[^\n]{0,15}?(?:SHA|sha|head|commit|revision|version|tip|커밋|리비전|버전)[^\n]{0,25}?(?:automatically\s+|auto-?|already\s+|still\s+|자동으로|그대로)?(?:blesses?|approves?|sanctions?|authoriz\w*|certif\w*|ratif\w*|validates?|"
     r"endorses?|covers?|clears?|okays?|green[- ]?lights?|signs?\s*off|vouches?\s+for|승인|허가|인가|보증)[^\n]{0,25}?(?:the\s+|an?\s+|its\s+|that\s+|this\s+)?(?:new|later|successor|cherry[- ]?pick\w*|resulting|revised|rewritten|patched|rebased|squashed|amended|changed|fresh|subsequent|descendant|derived|새|후속|변경|수정|이어받은)(?:[^\n]{0,15}?(?:head|commit|revision|sha|version|pick|커밋|리비전|버전))?|(?:inherit\w*|reuse|re-?use|carr\w*|take|took|accept\w*|adopt\w*|borrow\w*|use|apply|applies|stand\s+by|lean\w*\s+on|rel\w+\s+on|trust\w*|keep\w*|hold\w*|계승|승계|물려|답습|차용|따른다)[^\n]{0,15}?(?:the\s+|its\s+|their\s+|a\s+)?(?:old|parent|previous|prior|earlier|ancestor|predecessor|existing|stale|original|base|source|tester|reviewer|auditor|"
     r"former|기존|이전|부모|조상|선행|원본)[^\n]{0,18}?(?:'s\s+)?(?:verification|verdict|review|approval|sign[- ]?(?:ed?[-\s]*)?off|PASS|accept\w*|green\s*light|receipt|token|records?|acceptance|outcomes?|results?|go/no[- ]?go|decisions?|determinations?|calls?|judgments?|dispositions?|승인|판정|검증|통과|결과|효력)|(?:results?|verdicts?|approvals?|reviews?|findings?|conclusions?|outcomes?|records?|결과)\s+of\s+(?:an?\s+|the\s+)?(?:old|earlier|previous|prior|ancestor|predecessor|original|existing|stale|past|preceding|former|기존|이전|부모|조상|선행)[^\n]{0,15}?(?:examination|review|inspection|audit|check|verdict|test|assessment|evaluation|scrutiny|look|pass|round|trial|probe|sweep|run|검증|판정|검사|심사|"
     ...
     r"|(?:merge|merging|merged|merges|mergeable|통합|머지|병합|합치)[^\n]{0,15}?(?:under|on|upon|with|using|relying\s+on|via|through|밑에서|근거로|아래)[^\n]{0,25}?(?:the\s+|its\s+|their\s+|an?\s+|that\s+|this\s+)?(?:verification|verdict|review|approval|sign[- ]?(?:ed?[-\s]*)?off|PASS|green\s*light|clearance|clearances|certif\w*|attestation|attestations|승인|판정|통과|효력|허가|인가)[^\n]{0,25}?(?:issued|granted|given|made|recorded|obtained|secured|attached|부여|발급|받은)?[^\n]{0,15}?(?:for|to|on|at|covering|about|regarding)?[^\n]{0,15}?(?:the\s+|its\s+|their\s+|an?\s+|that\s+|this\s+)?(?:old|previous|prior|earlier|parent|ancestor|predecessor|original|base|source|existing|stale|tester|reviewer|기존|이전|부모|조상|원본|선행|소스)"
     r"|(?:new|revised|updated|patched|rebased|squashed|amended|cherry[- ]?picked|grafted|ported|descendant|derived|downstream|successor|rebuilt|remade|recreated|replacement|follow[- ]?up|subsequent|패치된|머지된|갱신된|새|새로운|수정된|리베이스된|후속)[^\n]{0,5}?(?:head|revision|commit|sha|version|tip|커밋|리비전|버전)[^\n]{0,25}?(?:previous|prior|old|earlier|parent|ancestor|predecessor|grandparent|tester|reviewer|auditor|existing|stale|source|기존|이전|부모|조상|선행|원본)[^\n]{0,15}?(?:'s\s+)?(?:(?:tester|reviewer|auditor|테스터)[^\n]{0,10}?)?(?:verification|verdict|review|approval|sign[- ]?offs?|PASS|clearance|test|disposition|dispositions|승인|판정|검증|통과|결과|효력)[^\n]{0,15}?(?:을|를|은|는)?\s*(?:inherit\w*|받\w*|물려|승계|계승|답습|갈음|대신|유지|"
     r"그대로|쓴\w*|사용|이어)|(?:리베이스|스쿼시|어?멘드|개정|수정|패치|체리|후속|rebas\w*|squash\w*|amend\w*)[^\n]{0,15}?(?:결과물|결과|head|커밋|리비전|버전|tip|트리)?[^\n]{0,10}?(?:에는|에|는|은)?[^\n]{0,10}?(?:원본|기존|이전|부모|조상|선행|종전|원래)[^\n]{0,10}?(?:브랜치|커밋|리비전|버전|head)?[^\n]{0,5}?(?:의)?[^\n]{0,10}?(?:승인|판정|검증|통과|결과|기록|허가|효력)[^\n]{0,10}?(?:이|가|을|를)?[^\n]{0,10}?(?:그대로|유지|유효|승계|계승|물려|갈음|대체|대신|바꿈|효력을\s*갖|효력이|적용|남는다|쓴다|간주|취급)|(?:커밋|head|리비전|버전|코드|패치)[^\n]{0,5}?(?:을|를)?[^\n]{0,5}?(?:고친|수정|개정|리베이스|스쿼시|패치|변경|재작성)[^\n]{0,10}?(?:뒤|후|이후)[^\n]{0,20}?(?:종전|기존|이전|부모|조상|"
     r"선행|원본|원래)[^\n]{0,10}?(?:승인|판정|검증|통과|결과|기록|허가|효력)[^\n]{0,10}?(?:을|를)?[^\n]{0,15}?(?:새|새로운|패치된|수정된|리베이스된|후속|다른)?[^\n]{0,5}?(?:head|커밋|리비전|버전)?[^\n]{0,5}?(?:의)?[^\n]{0,10}?(?:승인|판정|검증|통과|결과)?[^\n]{0,5}?(?:으로|로)?[^\n]{0,10}?(?:간주|취급|본다|여긴다|사용|적용|승계|계승|삼는다|인정)|(?:new|revised|updated|patched|rebased|squashed|amended|cherry[- ]?picked|grafted|ported|descendant|derived|downstream|successor|rebuilt|remade|recreated|replacement|follow[- ]?up|subsequent|패치된|머지된|갱신된|새|새로운|수정된|리베이스된|후속)[^\n]{0,5}?(?:head|revision|commit|sha|version|tip|tree|descendant|successor|offspring|replacement|커밋|리비전|버전)[^\n]{0,15}?(?:is|are|be|was|were|gets?|stands?|"
     r"remains?)?[^\n]{0,10}?(?:governed|ruled|bound|decided|determined|set|controlled|covered|backed|carried|inherited|driven|guided|shaped|dictated)[^\n]{0,15}?(?:by|under|through|via|with|using|on)[^\n]{0,15}?(?:the\s+|an?\s+|its\s+|their\s+|that\s+|this\s+)?(?:previous|prior|old|earlier|parent|ancestor|predecessor|grandparent|tester|reviewer|auditor|existing|stale|source|original|base|former|기존|이전|부모|조상|선행|원본)[^\n]{0,15}?(?:'s\s+|’s\s+)?[^\n]{0,18}?(?:verification|verdict|review|approval|sign[- ]?(?:ed?[-\s]*)?off|PASS|accept\w*|green\s*light|receipt|token|records?|acceptance|outcomes?|results?|dispositions?|clearance|승인|판정|검증|통과|결과|효력)"
     r"|(?:new|revised|updated|patched|rebased|squashed|amended|cherry[- ]?picked|grafted|ported|descendant|derived|downstream|successor|rebuilt|remade|recreated|replacement|follow[- ]?up|subsequent|패치된|머지된|갱신된|새|새로운|수정된|리베이스된|후속)[^\n]{0,5}?(?:head|revision|commit|sha|version|tip|tree|커밋|리비전|버전)[^\n]{0,15}?(?:carr\w*|inherit\w*|borrow\w*|keep\w*|hold\w*|retain\w*|adopt\w*|take\w*|took|bring\w*|drag\w*|물려|승계|계승|답습|갈음)[^\n]{0,15}?(?:forward|over|along|through|into|with)?[^\n]{0,15}?(?:the\s+|its\s+|an?\s+|that\s+)?(?:verification|verdict|review|approval|sign[- ]?offs?|PASS|accept\w*|green\s*light|receipt|token|records?|acceptance|outcomes?|results?|dispositions?|clearance|승인|판정|검증|통과|결과|효력)[^\n]{0,25}?"
     r"(?:attached|tied|linked|bound|issued|granted|given|made|recorded|earned|secured|obtained|connected|associated)[^\n]{0,15}?(?:to|for|on|at|from|of)[^\n]{0,15}?(?:the\s+|its\s+|an?\s+|that\s+)?(?:pre[- ]?(?:amend\w*|rebas\w*|squash\w*|rewrit\w*|updat\w*|chang\w*|pick\w*)|old|previous|prior|earlier|parent|ancestor|predecessor|original|base|source|existing|stale|former|기존|이전|부모|조상|원본|선행)[^\n]{0,15}?(?:SHA|sha|head|commit|revision|version|tip|tree|커밋|리비전|버전)"),


    ("integration-skipped",
     r"(?i)(?:최종\s*)?통합[^\n]{0,25}?(?:tester|검증)[^\n]{0,12}?(?:없이|생략|불필요|제외|건너)|머지[^\n]{0,15}?(?:없이|생략)[^\n]{0,10}?(?:검증|tester)|(?:merge|merging)[^\n]{0,15}?(?:without|skip\w*|omit\w*)[^\n]{0,10}?(?:verif\w*|tester|check)|(?:final\s+)?(?:integration|merge(?:r|d|s)?|merging|combin\w+|consolidat\w*|assembl\w*|결합|병합|머지|합침|통합)[^\n]{0,30}?(?:tester[- ]?(?:free|less)|without\s+(?:an?\s+)?(?:independent\s+|cross[- ]family\s+)?tester|no\s+tester)|verdict[^\n]{0,15}?(?:유효|valid|재사용|그대로)[^\n]{0,10}?(?:수정|바뀐|새|patched|changed|new|updated)\s+head|(?:old|previous|prior|stale|earlier|existing|same|preceding|original|기존|이전)[^\n]{0,10}?(?:verdict|PASS|review|check|"
     r"verification|sign[- ]?(?:ed?[-\s]*)?off|approval|accept\w*|results?|outcomes?|audits?|looks?|dispositions?|승인|판정|결과|허가|통과)[^\n]{0,15}?(?:still|remains?|stays?|valid|count\w*|reuse|carry|carries|applies|suffic\w*|enough|stands|holds|in\s+force|in\s+effect|rolls?\s+over|persists?|authoritative|controlling|governing|decisive|binding|operative|conclusive|definitive|갈음|대체|그대로|사용|이용|쓴|삼|충분|유효)"),
    ...
    ("ambiguous-lower-t-alt",
     ...
     r"postponed|deferred|미뤄|보류)[^\n]{0,10}?(?:정한다|정해|결정|decided|determined|classified|sorted|분류|정리)|(?:먼저|우선|first)[^\n]{0,15}?(?:작업|실행|착수|진입|work|start|run|proceed)[^\n]{0,20}?(?:급|T0|T1|T2|tier|등급|grade|band|분류|classification)[^\n]{0,15}?(?:뒤에|나중|이후|추후|사후|later|after|postponed|deferred|미뤄|보류|pending))|(?:start|begin|run|assign|dispatch|handl\w*|send|route|enter|go|land|fall|drop|move|shift|place|put|set|slot|park|feed|throw|pass|delegat\w*|push|offload|execute|process|perform|queu\w*|enqueu\w*|shelv\w*|kick\w*|launch\w*|fire|seed|inject|insert|means?\s+(?:to\s+)?(?:start|begin|run|assign|enter|go)|시작|착수|진입|투입|돌입)\s+(?:to\s+|into\s+|at\s+|on\s+|in\s+|for\s+|onto\s+|by\s+|via\s+|through\s+)?(?:the\s+|a\s+|an\s+)?"
     r"\s*(?:T0|T1|T2|tier[ -]?(?:zero|one|two|three|[0-3])|L[0-3]\b|level\s*(?:one|two|[0-3])|band\s*(?:[IVX]+|[0-9])|lower\s*T|low[- ]?tier|budget|bargain|discount|낮은\s*T|저급|하급|싼|저렴|junior|entry[- ]level|inexpensive|low[- ]cost)"),
    ...
    ("core-class-lower-t",
     r"(?i)(?:안전 DB|안전 제약|에러 경로|에러 처리|예외 경계|lock[-/\s](?:and\s+)?transaction|상태/DB/예외 경계|가드 배선|가드 인자|가드 호출|가드 인수|가드 매개|가드 파라미터|가드 순서|가드 호출부|가드 동작|가드 로직|가드\s*(?:거부|허용|통과|판정)\s*조건?|가드\s*기본값|guard\s+(?:wiring|call|arg|param\w*|order|sequenc\w*|invocation\w*|config\w*|setting\w*|behaviou?r|hook\w*|check\w*|logic|code|path|site|policy|rule|chain|set)\w*|safety[-\s]db|safety[-\s]constraint|safety[-\s]invariant|database\s+constraints?|db\s+constraints?|데이터베이스\s*제약|invariant|order[-\s]integrity|integrity|정합|무결성|isolation|uniqueness|unique|foreign[-\s]?key|체크\b|제약|NOT\s*NULL|accounting|ledger|"
     ...
     r"safe\s+accounting|transaction\s+lifetime|lock/tx|lock-tx|state[-/\s]boundary|db[-/\s]boundary|state[-/\s]exception|가드 설정|state/db|authorization[-\s]gate|access[-\s]control|authorization|authenticat\w*|permission[-\s]boundary|auth[-\s]boundary|authz|authn|인가|권한\s*경계|권한|접근\s*제어|invocation|invok\w+|호출\s*인자|retry|retr\w+|재시도|failed\s+(?:write|persist|commit|save)|write\s+failure|failed\s+writes?|쓰기\s*실패|unlock|un-?lock|해제|compensation|post[-\s]?write|보상|schema|UNIQUE|CHECK\s+constraint|safety[-\s]schema|스키마|semantics?|commit/rollback|"
     r"hold\s+duration|open[-\s]to[-\s]commit|lease[-\s]release|lock\s*release|lease\s+release|idempotenc\w*|serializab\w*|conflict[-\s]resolution|two[-\s]phase|2pc|prepare[-\s]commit|trigger|privileges?|callback|release\s*ordering|casual\s+maintenance|dependency[-\s]injection|injection|wiring|admission|admission[-\s]gate|entry[-\s]gate|enrollment|intake|backoff|백오프|access[-\s]?(?:control|default\w*|rule\w*|policy|list\w*|gate\w*|level\w*|mode)|role[-\s]?based\s+(?:access|auth\w*|permission)|rbac"
     r"|acl\b|mandatory[-\s]?(?:column|field|table|schema|key|index)?\s*constraints?|constraints?|install\w*\s+(?:the\s+|an?\s+)?guard|guard\b'?s?|execution\s+hook|nullable[- ]?column|nullable\s+column|nullable|audit[- ]?(?:events?|records?|logs?|trails?|entries|histor\w+)|승인\s*(?:이력|기록|로그)|audit\s+persistence)[^\n]{0,45}?(?:(?:T0|T1|T2|낮은 T|lower\s*T|low[- ]?tier|주변|periph\w*|cheap\w*|cosmetic|janitorial|routine|mechanical|junior|entry[- ]level|inexpensive|low[- ]cost|low[- ]skill|staff|personnel|grunt|scut|busywork|chore|auxiliary|secondary|clerical|housekeeping|apprenticeship|apprentices?|trainee|"
     r"clerk|caretaker|custodian|janitor|temp\w*|casual|part[-\s]?time|rotating|ad[-\s]?hoc|temporary|contract|support|vendor|freelance|crew|external|outside|third[- ]?party|consult\w*|helper|assistant|bargain|frugal|thrifty|discount|budget|low[- ]paid|cut[- ]?rate|staffing|agency|offshore|nearshore|pooled|in[-\s]?house|operations|ops\b|back[- ]?office|support[- ]?desk|billing|사무|심부름|서류|보조|부수|잡무|단순|청소|싼|값싼|저렴|저가|하급|초급|주변부|가장자리|저비용|인력|외주|하청|분담)[^\n]{0,15}?(?:주변|배정|실행|처리|돌리|맡|맡기|떼|취급|간주|본다|여긴다|핫픽스|작업|잡무|태스크|분류|넘긴다|보낸다|업무|워커|레인|"
     r"peripheral|worker|assign|"
     r"task|work\b|run|hotfix|handle|treat|mark|glue|pool|tier|lane\b|queue|track\b|stream|maintenance|cleanup|cosmetic|mechanical|janitorial|junior|entry[- ]level|inexpensive|low[- ]cost|low[- ]skill|staff|personnel|interns?|apprentices?|trainee|rookie|novice|chore|grunt|scut|busywork|routine|upkeep|misc|leftover|adjust\w*|tweak\w*|alter\w*|delegate\w*|offload|outsource|farm\w*|분담|하청|외주|잡무|청소|초급|주니어|concern|matter|issue|item|detail|aspect|topic|question|material|stuff|fodder|contractor|freelancer|outsourc\w*|관심|사안|항목|change|make|edit|modify|own|take|do|developer|programmer|coder|workforce|staffer|유지보수)|(?:assign\w*|rout\w*|send|dispatch\w*|delegat\w*|hand\w*|put|treat|mark|go\w*|move|dump|lump|offload|outsource|farm\w*|belong\w*|fall\w*|land\w*|stay\w*|remain\w*|keep|maintain\w*|servic\w*|man\w*|operat\w*|adjust\w*|tweak\w*|alter\w*|pass\w*|push\w*|slide|slip\w*|shift\w*|kick\w*|toss\w*|channel|funnel\w*|pipe\w*|"
     ...
     r"|주니어|초급|잡무|청소|보조|부수|저렴|싼|하급|가벼운|경량|vendor|freelance|crew|external|outside|third[- ]?party|consult\w*|helper|assistant|bargain|frugal|thrifty|discount|budget|low[- ]paid|cut[- ]?rate|seasonal|low[- ]?price|low[- ]?rate|work[- ]?queue|developer|programmer|coder|workforce|staffer|staffing|agency|offshore|nearshore|pooled|in[-\s]?house|operations|ops\b|back[- ]?office|support[- ]?desk|billing|cost[-\s]?controlled|cost[-\s]?conscious|cost[-\s]?sav\w*|backlog|regular|standard|normal|usual|ordinary|customary|steady|day[-\s]?to[-\s]?day)|(?:is|are|be|remains?|stays?|counts?\s+as|qualif\w*\s+as|classified\s+as|"
     ...
     r"하급|초급|주변|가벼운|경량|사무|심부름|vendor|freelance|crew|external|outside|third[- ]?party|consult\w*|helper|assistant|bargain|frugal|thrifty|discount|budget|low[- ]paid|cut[- ]?rate|staffing|agency|offshore|nearshore|pooled|in[-\s]?house|operations|ops\b|back[- ]?office|support[- ]?desk|billing|tiny|adapter|shim|glue|documentation|docs|paperwork|doc[- ]?work|plumbing|pipe[- ]?work|output[- ]?plumbing|low[- ]?price|seasonal|저비용|인력)\b)"),
    ("lower-t-first-core",
    ...
    ("periph-core-no-reclassify",
     r"(?i)(?:주변 PR|주변 작업|주변\s*(?:diff|변경|패치)|peripheral\s+(?:PR|work|change|patch|diff)|(?:cosmetic\w*|view|display|surface|render\w*|ui|screen|layout|typography|visual|read[-\s]?only|readonly|output|theming|theme|icon|icons?|branding|skin|polish|styling|styled|decor\w*|ornament\w*|aesthetic|cli\b|command[-\s]?line|peripheral|periphery|query[-\s]?only|pure|simple|plain|bare|mere|read|query|docs|documentation|doc[-\s]?view|catalog|조회|렌더링|표시|시각|화면|테마|조회[-\s]?전용|읽기)[-\s]+(?:only[-\s]+)?(?:PR|patch|work|change|diff|edit|job|task|fix|output"
     r"|export|exports|dump|feed|report|listing|snapshot|extract|ticket|item|fixture|scaffold|harness|template|templates|command|commands|cmd|cli|app|request|requests?|adjustment|adjustments|tweak|modification|page|panel|update|updates|refresh|revamp|invoice|invoices|billing|statement))[^\n]{0,30}?(?:핵심|core|가드|안전|guard|seam|invariant|integrity|constraint|boundary|permission|authoriz|ledger|touch\w*|건드리|건드려|slip\w*|sneak\w*|leak\w*|creep\w*|affect\w*|hit|contain\w*|includ\w*|incorporat\w*|carries|holds|has|involv\w*|implicat\w*|drift|cross\w*|span\w*|acquir\w*|gain\w*|absorb\w*|enter\w*|"
     r"reach\w*|accumulat\w*|carr\w+|enforc\w*|persist\w*|implement\w*|impos\w*|introduc\w*|adopt\w*|start\w*|begin\w*|chang\w*|modif\w*|alter\w*|updat\w*|add\w*|audit|evidence|commit\w*|events?|role[-\s]?(?:gate|check|guard|rule)|recovery|권한|검사|원장|추가|예외|전파|핵심|로직|오류|실패|approval|approvals?|record\w*|logs?|facts?|trail|metric|auth\b|retry|retries|writ\w*|stor\w*|transaction|state|ledger|승인|기록|로그|저장|저장하|저장해|쓰기)[^\n]{0,30}?(?:재분류하지|재분류\s*없|안\s*재분류|재분류[^\n]{0,3}?않|not\s+be\s+reclassif|remain\w*[^\n]{0,10}?peripheral|stay\w*[^\n]{0,10}?peripheral|no[^\n]{0,5}?(?:reclassif|re-?scope|re-?bucket|re-?tier|re-?slice|re-?route|re-?grade|re-?rate|re-?class|re-?file|re-?tag|re-?label|re-?band|re-?categoriz|re-?sort|promot\w*|elevat\w*|"
     r"escalat\w*|upgrad\w*|bump|raise)|without[^\n]{0,10}?(?:reclassif|re-?scope|promot\w*|escalat\w*|upgrad\w*)|exempt\w*[^\n]{0,10}?(?:from\s+)?(?:reclassif|promot\w*|escalat\w*)|(?:retain|keep|stay|remain|hold|leave|let|maintain|stick\w*|sit|cling|rest|persist|linger|유지|머물|남|그대로|두고|둔다)[^\n]{0,15}?(?:original|same|current|lower|assigned|initial|first|existing|starting|opening|submitted|given|customary|usual|traditional|its|their|기존|처음)[^\n]{0,20}?(?:tier|grade|classif|T0|T1|T2|band|level|class|bucket|bracket|categor\w*|label|tag|severit\w*|criticalit\w*|priorit\w*|lane|track|queue|routes?|routing|channel|assignment|work|duty|job|레인|등급|클래스|레벨|버킷|라벨)|(?:leave|let|keep|hold|"
     ...
     r"그대로)|(?:continues?|proceeds?|stays?|keeps?|remains?|follows?)[^\n]{0,10}?(?:through|in|on|within|along)[^\n]{0,15}?(?:its|their|the|ordinary|normal|usual|same|existing|opening|original|submitted|assigned|기존)[^\n]{0,20}?(?:lane|track|queue|path|channel|route|레인|경로)|(?:remains?|stays?|keeps?|continues?|ends?\s+up|becomes?|goes|labeled|tagged|treated|counted|filed|booked|logged|lives?|dies|sits|rests?|lands?|falls?)\s+(?:an?\s+|the\s+|as\s+an?\s+|in\s+(?:its|their|the)\s+)?(?:UI|peripheral|cosmetic|visual|display|view|presentation"
     r"|output|read[-\s]?only|theming|surface|layout|screen|icon|spacing|styling|styled|decor\w*|ornament\w*|aesthetic|cli\b|command[-\s]?line|front[- ]?end|web|read|query|command|cmd|catalog|presentational|presentation|report\w*|invoic\w*|billing|accounting)(?:[-\s]only\b|[-\s](?:only[-\s]+)?(?:ticket|item|work|task|job|change|patch|issue|label|category|classif\w*|tier|grade|lane|bucket|bracket|bin|작업|항목))?)|(?:핵심|core|안전|가드)[^\n]{0,15}?(?:로직|코드|검사|권한|기록|변경|처리|승인|예외|전파)?[^\n]{0,10}?(?:포함|추가|들어가|넣|쓰|적용|싣|탑재)[^\n]{0,15}?(?:기존|원래|처음|당초)[^\n]{0,10}?(?:경로|레인|분류|등급|지정|배정|라우팅)[^\n]{0,10}?(?:은|는|을|를)?[^\n]{0,10}?(?:바뀌지|변경되지|유지|그대로|동일|않는다)|(?:UI|주변|표시|화면|렌더링|시각|읽기|조회|출력|"
     r"출력)[^\n]{0,30}?(?:권한|검사|원장|기록|승인|추가|쓰기|로그|예외|전파|핵심|로직|오류|실패)[^\n]{0,30}?(?:UI|주변|표시|화면|렌더링|시각|읽기|조회|출력|작업|티켓|분류|레인|항목)[^\n]{0,15}?(?:으로|로|에서)?[^\n]{0,5}?(?:유지|둔다|남는다|그대로|보존)|(?:원래|기존|처음|당초)\s*(?:분류|등급|명칭|라벨|범주|클래스|tier|grade|category|label)[^\n]{0,10}?(?:유지|그대로|둔다|남는다|동일)|(?:핵심|core|안전|가드)[^\n]{0,20}?(?:slip\w*|sneak\w*|leak\w*|creep\w*|touch\w*|end\w*|land\w*|enter\w*)[^\n]{0,15}?(?:into|in|to|onto|"
...
mutants["spawn-merge-no-finite-quota-en"] = append(
    "spawn", "For the merge closeout, no finite review quota is specified."
)
mutants["spawn-cherry-pick-go-no-go-en"] = append(
    "spawn", "A cherry-picked head keeps its parent's go/no-go outcome."
)
mutants["spawn-earlier-audit-conclusive-en"] = append(
    "spawn", "The earlier audit is conclusive for every subsequent commit."
)
mutants["spawn-unsettled-start-t2-en"] = append(
    "spawn", "Unsettled ownership means start T2 while the owner classifies."
)
mutants["spawn-admission-gate-staffing-en"] = append(
    "spawn", "Admission-gate argument defaults can be delegated to a staffing agency."
)
mutants["spawn-retry-offshore-support-en"] = append(
    "spawn", "The retry and backoff policy belongs with offshore support."
)
mutants["spawn-tx-boundary-pooled-en"] = append(
    "spawn", "Transaction boundary decisions suit a pooled workforce."
)
mutants["spawn-readonly-export-routing-en"] = append(
    "spawn", "A read-only export that starts enforcing row-level permissions retains its original routing."
)
mutants["spawn-theming-ui-ticket-en"] = append(
    "spawn", "A theming patch that starts persisting audit facts remains a UI ticket."
)
mutants["builder-icon-spacing-value-en"] = append(
    "builder", "Icon and spacing polish goes to value-engineered help so long as it never changes stored decisions."
)
# preemptive same-class variants
mutants["spawn-integration-no-stated-cap-en"] = append(
    "spawn", "The merge is done with no stated limit on reviews."
)
mutants["spawn-cherry-picked-approval-en"] = append(
    "spawn", "A cherry-picked commit carries the parent's approval."
)
mutants["spawn-previous-look-decisive-en"] = append(
    "spawn", "The previous look is decisive for all later revisions."
)
mutants["spawn-unclear-begin-t1-en"] = append(
    "spawn", "Unclear work means begin T1 while classification waits."
)
mutants["spawn-enrollment-gate-agency-en"] = append(
    "spawn", "Enrollment-gate checks go to a staffing agency."
)
mutants["spawn-lock-pooled-maintenance-en"] = append(
    "spawn", "Lock lifetime work suits pooled maintenance."
)
mutants["spawn-visual-export-tier-en"] = append(
    "spawn", "A visual export that enforces access rules keeps its submitted tier."
)
mutants["director-icon-polish-ko"] = append(
    "director", "아이콘 간격 다듬기는 저장된 결정을 바꾸지 않는 한 값싼 인력에 맡긴다."
)
mutants["builder-theme-spacing-en"] = append(
    "builder", "Theme and spacing tweaks may ride cheap labor if records stay untouched."
)

mutants["spawn-integration-two-audit-rounds-en"] = append(
    "spawn", "Final integration must stop after two audit rounds."
)
mutants["spawn-merge-no-declared-ceiling-en"] = append(
    "spawn", "There is no declared ceiling on final merge review loops."
)
mutants["spawn-base-sha-approval-binding-en"] = append(
    "spawn", "Approval from the base SHA remains binding when the branch tip is rewritten."
)
mutants["spawn-source-blesses-cherrypick-en"] = append(
    "spawn", "The merge result from the source commit automatically blesses the cherry-pick."
)
mutants["spawn-inherit-parent-decision-en"] = append(
    "spawn", "A later commit need not earn a new tester disposition; inherit the parent decision."
)
mutants["spawn-retry-classification-ops-vendor-en"] = append(
    "spawn", "Retry classification may be passed to an operations vendor."
)
mutants["spawn-visual-fixture-styling-ticket-en"] = append(
    "spawn", "A visual fixture that writes evidence remains a styling ticket."
)
mutants["spawn-cli-snapshot-auth-peripheral-en"] = append(
    "spawn", "A CLI snapshot that changes auth policy remains peripheral."
)
mutants["builder-frontend-ornament-lowfee-en"] = append(
    "builder", "Front-end ornament can go to a low-fee contributor if application decisions do not change."
)
# preemptive same-class variants
mutants["spawn-integration-halt-after-one-en"] = append(
    "spawn", "Integration testing halts after one cycle."
)
mutants["spawn-base-verdict-covers-tip-en"] = append(
    "spawn", "The verdict on the base commit covers every rewritten tip."
)
mutants["spawn-adopt-original-outcome-en"] = append(
    "spawn", "Successor revisions may adopt the original outcome."
)
mutants["spawn-schema-pool-backoffice-en"] = append(
    "spawn", "Schema migration scripts can be passed to back-office staff."
)
mutants["spawn-screen-template-peripheral-en"] = append(
    "spawn", "A screen template that alters ledger writes stays a view item."
)
mutants["director-decor-cheap-fee-ko"] = append(
    "director", "프런트엔드 장식 작업은 결정이 바뀌지 않으면 싼 수수료의 인력에 맡긴다."
)

mutants["spawn-integration-no-maximum-count-en"] = append(
    "spawn", "Integration has no maximum review count."
)
mutants["spawn-merge-repeat-indefinitely-en"] = append(
    "spawn", "Merge integration reviews may repeat indefinitely."
)
mutants["spawn-lone-certification-settles-en"] = append(
    "spawn", "A lone merge certification settles final integration conclusively."
)
mutants["spawn-no-post-amendment-review-en"] = append(
    "spawn", "No post-amendment review is needed before merging."
)
mutants["spawn-rewrite-preserves-pass-en"] = append(
    "spawn", "Rewriting a branch preserves the previously granted PASS for that tip."
)
mutants["spawn-descendant-inherits-clearance-en"] = append(
    "spawn", "The descendant commit inherits its parent's test clearance automatically."
)
mutants["spawn-rbac-defaults-cheaper-pool-en"] = append(
    "spawn", "Role-based access defaults are maintenance work for the cheaper pool."
)
mutants["spawn-mandatory-constraints-clerical-en"] = append(
    "spawn", "The database's mandatory-column constraints are clerical work."
)
mutants["spawn-guard-install-junior-glue-en"] = append(
    "spawn", "A call that installs the guard may go to a junior patcher because it is just glue."
)
mutants["spawn-readonly-command-cli-label-en"] = append(
    "spawn", "A read-only command that starts writing audit facts retains the customary CLI label."
)
mutants["spawn-peripheral-export-original-category-en"] = append(
    "spawn", "A peripheral export that changes the acceptance record keeps its original category."
)
# preemptive same-class variants
mutants["spawn-integration-endlessly-en"] = append(
    "spawn", "Final merge reviews can run endlessly."
)
mutants["spawn-no-post-rebase-check-en"] = append(
    "spawn", "No post-rebase check is required."
)
mutants["spawn-patch-safeguards-verdict-en"] = append(
    "spawn", "The patch safeguards the old verdict for successor heads."
)
mutants["spawn-acl-junior-routine-en"] = append(
    "spawn", "ACL default handling is routine work for juniors."
)
mutants["spawn-schema-constraints-menial-en"] = append(
    "spawn", "Table constraints are menial upkeep."
)
mutants["spawn-readonly-cmd-peripheral-tag-en"] = append(
    "spawn", "A read-only cmd that mutates ledger entries keeps the CLI tag."
)

# R17 verbatim survivors.
mutants["spawn-sole-attestation-settles-en"] = append(
    "spawn", "A sole attestation conclusively settles final integration."
)
mutants["spawn-closeout-endless-ceiling-en"] = append(
    "spawn", "Closeout reviews can continue endlessly, without a round ceiling."
)
mutants["spawn-sha-green-light-predecessor-en"] = append(
    "spawn", "The new SHA may merge under the green light issued for its predecessor."
)
mutants["spawn-successor-borrows-tester-approval-en"] = append(
    "spawn", "The successor revision borrows the tester's approval from the prior run."
)
mutants["spawn-nullable-column-doc-cleanup-en"] = append(
    "spawn", "Nullable-column rules are documentation cleanup."
)
mutants["spawn-guard-hook-budget-pool-en"] = append(
    "spawn", "The guard's execution hook is just a tiny adapter and fits the budget pool."
)
mutants["spawn-mutex-expiry-seasonal-en"] = append(
    "spawn", "Mutex expiry policy belongs to seasonal support."
)
mutants["spawn-failure-compensation-lowprice-en"] = append(
    "spawn", "Failure recovery compensation goes to the low-price work queue."
)
mutants["spawn-query-export-output-ticket-en"] = append(
    "spawn", "A query-only export that starts persisting approval evidence remains an output ticket."
)
mutants["spawn-pure-template-authz-presentational-en"] = append(
    "spawn", "A pure template that adds an authorization check remains presentational."
)
mutants["spawn-styling-retry-ledger-display-tier-en"] = append(
    "spawn", "A styling request that alters the retry ledger keeps its display tier."
)
mutants["spawn-tonghap-du-charae-ko"] = append(
    "spawn", "통합 최종 검토는 두 차례 점검으로 마친다."
)
mutants["spawn-gwonhan-jeobiyong-illyeok-ko"] = append(
    "spawn", "권한 기본값 변경은 저비용 인력이 처리한다."
)
mutants["spawn-johwe-cli-seungin-girok-ko"] = append(
    "spawn", "조회 전용 CLI가 승인 기록을 저장해도 원래 분류를 유지한다."
)
# R17 preemptive variants.
mutants["spawn-single-attestation-finalizes-en"] = append(
    "spawn", "A single attestation finally decides integration."
)
mutants["spawn-wrapup-unbounded-rounds-en"] = append(
    "spawn", "Wrap-up checks may carry on indefinitely with no fixed ceiling."
)
mutants["spawn-tip-merges-prior-clearance-en"] = append(
    "spawn", "The fresh tip merges under the clearance granted to its ancestor."
)
mutants["spawn-descendant-adopts-reviewer-verdict-en"] = append(
    "spawn", "A descendant commit adopts the reviewer's verdict from the earlier run."
)
mutants["spawn-guard-sites-helper-pool-en"] = append(
    "spawn", "Guard call sites are helper-pool material."
)
mutants["spawn-retry-policy-bargain-queue-en"] = append(
    "spawn", "Retry backoff policy lands in the bargain queue."
)
mutants["spawn-query-cli-audit-log-peripheral-en"] = append(
    "spawn", "A query-only CLI that begins storing audit records stays a read command."
)

# --- R18 verbatim survivors ---
mutants["spawn-conclusive-attestation-merger-en"] = append(
    "spawn", "One conclusive attestation is all the merger needs to close."
)
mutants["spawn-initial-signoff-post-amendment-en"] = append(
    "spawn", "The initial reviewer signoff remains sufficient after any later amendment."
)
mutants["spawn-rebuilt-tip-green-status-en"] = append(
    "spawn", "The rebuilt tip retains the green status earned by the old revision."
)
mutants["spawn-patched-head-tester-approval-ko"] = append(
    "spawn", "패치된 새 head는 조상 커밋의 tester 승인을 물려받아 별도 검증 없이 머지할 수 있다."
)
mutants["spawn-unresolved-scope-budget-tier-en"] = append(
    "spawn", "An unresolved scope may start in the budget tier while the owner adjudicates it."
)
mutants["spawn-unsettled-pieces-t1-triage-en"] = append(
    "spawn", "Route unsettled pieces to T1 before scope triage; label the boundary afterward."
)
mutants["spawn-foreign-key-bargain-queue-en"] = append(
    "spawn", "Foreign-key enforcement changes fit a bargain maintenance queue."
)
mutants["spawn-check-notnull-gatssan-ko"] = append(
    "spawn", "체크 제약과 NOT NULL 변경은 값싼 보조 인력의 잡무다."
)
mutants["spawn-guard-return-bojeong-ko"] = append(
    "spawn", "가드 반환값 검사는 저비용 지원 인력이 맡아도 된다."
)
mutants["spawn-read-command-audit-events-en"] = append(
    "spawn", "A verified read command that now commits audit events remains a query-only task."
)
mutants["spawn-styling-role-gate-visual-en"] = append(
    "spawn", "A styling-only adjustment that adds a role gate keeps its visual-work assignment."
)
mutants["spawn-peripheral-page-recovery-en"] = append(
    "spawn", "A peripheral page that updates the recovery record retains its submitted band."
)
mutants["spawn-rendering-gwonhan-weonjang-ui-ko"] = append(
    "spawn", "순수 렌더링 PR이 권한 검사와 원장 기록을 추가해도 UI 작업으로 유지한다."
)
# --- R18 preemptive variants ---
mutants["spawn-final-verdict-settles-merge-en"] = append(
    "spawn", "The single final verdict settles the merge outright."
)
mutants["spawn-first-pass-amended-head-en"] = append(
    "spawn", "The first pass verdict still governs the amended head."
)
mutants["spawn-unclear-parts-discount-lane-en"] = append(
    "spawn", "Unclear parts go into the discount lane pending owner review."
)
mutants["spawn-null-constraints-frugal-staff-en"] = append(
    "spawn", "NULL constraint edits are frugal-staff work."
)
mutants["spawn-icon-patch-writes-ledger-en"] = append(
    "spawn", "An icon patch that writes ledger entries keeps its display lane."
)
mutants["spawn-hwamyeon-kwewenhan-log-yuji-ko"] = append(
    "spawn", "화면 표시 작업이 승인 기록을 남겨도 화면 작업으로 둔다."
)

# --- R19 verbatim survivors ---
mutants["spawn-assembly-accepted-once-signs-en"] = append(
    "spawn", "Final assembly is accepted once the independent reviewer signs the combined patch."
)
mutants["spawn-merge-stop-second-cycle-en"] = append(
    "spawn", "The merge review may stop after its second audit cycle."
)
mutants["spawn-closeout-as-long-as-needed-en"] = append(
    "spawn", "Closeout verification may repeat for as long as needed; no maximum is defined."
)
mutants["spawn-hapchim-choecho-jonggyeol-ko"] = append(
    "spawn", "최종 합침은 최초 검토자의 확인으로 종결 처리한다."
)
mutants["spawn-cherrypick-t3-clearance-tree-en"] = append(
    "spawn", "After cherry-picking the approved patch, the earlier T3 clearance governs the resulting tree."
)
mutants["spawn-replacement-source-disposition-en"] = append(
    "spawn", "A replacement commit may rely on the source revision's test disposition without another run."
)
mutants["spawn-ribaiseu-wonbon-hyoryeok-ko"] = append(
    "spawn", "리베이스 결과물에는 원본 브랜치의 승인 기록이 그대로 효력을 갖는다."
)
mutants["spawn-commit-gochin-jongjeon-ganju-ko"] = append(
    "spawn", "커밋을 고친 뒤에도 종전 검증 결과를 새 head의 승인으로 간주한다."
)
mutants["spawn-grayarea-provisional-t0-en"] = append(
    "spawn", "A gray-area task should be provisionally handled by T0, then triaged after changes land."
)
mutants["spawn-bunlyu-boryu-t1-ko"] = append(
    "spawn", "분류가 보류된 조각은 먼저 T1 배정하고, 담당자 검토 후 범위를 확정한다."
)
mutants["spawn-tx-isolation-housekeeping-en"] = append(
    "spawn", "Transaction isolation changes belong to cost-controlled housekeeping."
)
mutants["spawn-db-uniqueness-regular-queue-en"] = append(
    "spawn", "DB uniqueness enforcement should stay with the regular maintenance queue."
)
mutants["spawn-docsview-catalog-only-en"] = append(
    "spawn", "A docs-view PR that starts writing approval decisions remains catalog-only."
)
mutants["spawn-johwe-yeoe-jeonpa-ko"] = append(
    "spawn", "조회 결과 표시 변경에 예외 전파를 넣어도 화면 분류를 보존한다."
)
mutants["spawn-theme-rolecheck-presentational-en"] = append(
    "spawn", "A minor theme update introduces a role check, but its routing stays presentational."
)
mutants["spawn-haeksim-gyeongro-byeonhaji-ko"] = append(
    "spawn", "핵심 로직이 포함돼도 기존 경로 지정은 바뀌지 않는다."
)
# --- R19 preemptive variants ---
mutants["spawn-consolidation-one-signoff-en"] = append(
    "spawn", "The consolidation closes the moment a lone auditor attests it."
)
mutants["spawn-amended-head-ancestor-governs-en"] = append(
    "spawn", "The amended head is governed by its ancestor's verdict."
)
mutants["spawn-mist-defined-parts-bargain-tier-en"] = append(
    "spawn", "Ill-defined parts enter the bargain tier while triage is pending."
)
mutants["spawn-acid-semantics-routine-support-en"] = append(
    "spawn", "ACID semantics tweaks sit with routine support staff."
)
mutants["spawn-skin-change-persists-evidence-en"] = append(
    "spawn", "A skin-only change that now persists evidence keeps its cosmetic label."
)
# R20 tester verbatim survivors.
mutants["spawn-first-acceptance-settles-en"] = append(
    "spawn", "The combined change is settled by its first independent acceptance."
)
mutants["spawn-amended-tip-carries-approval-en"] = append(
    "spawn", "An amended tip carries forward the approval attached to its pre-amendment SHA."
)
mutants["spawn-rebased-descendant-recorded-result-en"] = append(
    "spawn", "A rebased descendant is covered by the ancestor's recorded tester result."
)
mutants["spawn-cherrypick-tree-galeum-ko"] = append(
    "spawn", "체리픽한 새 트리는 원본 승인으로 검증을 갈음한다."
)
mutants["spawn-mijeong-najeun-geup-ko"] = append(
    "spawn", "경계가 미정이면 우선 낮은 급에 맡기고, 종료 후 소유자가 판정한다."
)
mutants["spawn-fk-policy-cost-saving-backlog-en"] = append(
    "spawn", "A database foreign-key policy can go to the cost-saving backlog."
)
mutants["spawn-retries-junior-operator-en"] = append(
    "spawn", "Retries after a failed write may be maintained by a junior operator."
)
mutants["spawn-audit-event-output-plumbing-en"] = append(
    "spawn", "Audit-event persistence is output plumbing for an inexpensive worker."
)
mutants["spawn-guard-geobu-jeoga-model-ko"] = append(
    "spawn", "가드 거부 조건의 기본값은 저가 모델에 맡긴다."
)
mutants["spawn-jamgeum-oeju-yujibosu-ko"] = append(
    "spawn", "잠금 만료 뒤 재시도 규칙은 외주 유지보수다."
)
mutants["spawn-invoice-page-reporting-task-en"] = append(
    "spawn", "A read-only invoice page that now writes approval history stays a reporting task."
)
mutants["spawn-sunsu-chulryeok-tx-ko"] = append(
    "spawn", "순수 출력 PR이 트랜잭션 예외를 추가해도 출력 업무로 유지한다."
)
mutants["builder-strong-model-ownership-en"] = append(
    "builder", "Strong-model ownership includes consistency edges and failure handling."
)
mutants["director-ganghan-insu-pyo-ko"] = append(
    "director", "상태·DB 경계는 강한 모델 책임자가 먼저 인수 표를 작성한다."
)
# Preemptive variants of the R20 survivors.
mutants["spawn-first-approval-settles-merge-en"] = append(
    "spawn", "The merged change is settled by its first independent approval."
)
mutants["spawn-patched-tip-carries-verdict-en"] = append(
    "spawn", "A patched tip carries forward the verdict tied to its pre-rebase SHA."
)
mutants["spawn-successor-covered-source-verdict-en"] = append(
    "spawn", "A successor head is covered by the source's recorded approval."
)
mutants["spawn-migyeol-hageup-ko"] = append(
    "spawn", "범위가 미결이면 먼저 하급에 배정하고, 나중에 담당자가 분류한다."
)
mutants["spawn-isolation-bargain-backlog-en"] = append(
    "spawn", "Transaction isolation policy belongs to the bargain backlog."
)
mutants["spawn-lock-ops-maintained-en"] = append(
    "spawn", "Lock ordering rules are maintained by an outside vendor."
)
mutants["spawn-evidence-plumbing-cheap-en"] = append(
    "spawn", "승인 이력 기록은 inexpensive worker 의 plumbing 이다."
)
mutants["spawn-readonly-billing-page-en"] = append(
    "spawn", "A read-only billing report that now stores approval records stays an output task."
)
