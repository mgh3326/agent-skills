...
FILE_FORBIDDEN = [
    ...
    r"명시된|선언된)\s*(?:ceiling|cap\w*|limits?|bounds?|maximum|max\b|quota|캡|상한|제한|쿼터)[^\n]{0,45}?(?:integration|merge(?:r|d|s)?|merging|combin\w+|결합|병합|머지|합침|통합|review|verification|검증|검토)"),
    ("verdict-inherit-after-change",
     ...
     r"stale|기존|이전|부모|조상|선행)[^\n]{0,15}?(?:'s\s+)?(?:verification|verdict|review|approval|sign[- ]?(?:ed?[-\s]*)?off|PASS|accept\w*|green\s*light|receipt|token|records?|acceptance|outcomes?|results?|go/no[- ]?go|go-no-go|decisions?|determinations?|calls?|judgments?|clearance|clearances|attestation|attestations|certif\w*|ratif\w*|tests?|testing|시험|테스트|status|state|green\s*status|승인|판정|검증|통과|결과|효력)|(?:squash\w*|amend\w*|rebas\w*|rewrit\w*|revis\w*|force[- ]?push|patch|updat\w*|cherry[- ]?pick\w*|picked|grafted|ported|transplanted|reapplied|restored|rebuilt|remade|recreated|repushed|체리|스쿼시|어?멘드|개정|리베이스|패치|수정|새\s*head|follow[- ]?up|subsequent|후속)[^\n]{0,25}?(?:inherit\w*|substitut\w*|replac\w*|reuse|re-?use|carr\w*|retain\w*|transfer\w*|grandfather\w*|rides?|borrow\w*|stands?\s+on|rests?\s+on|leans?\s+on|relies?\s+on|preserv\w*|saves?|safeguard\w*|conserv\w*|승계|계승|갈음|대신|대체|유지|"
     ...
     r"|(?:new|revised|updated|patched|rebased|squashed|amended|cherry[- ]?picked|grafted|ported|descendant|derived|downstream|successor|rebuilt|remade|recreated|follow[- ]?up|subsequent|패치된|머지된|갱신된|새|새로운|수정된|리베이스된|후속)[^\n]{0,5}?(?:head|revision|commit|sha|version|tip|커밋|리비전|버전)[^\n]{0,25}?(?:previous|prior|old|earlier|parent|ancestor|predecessor|grandparent|tester|reviewer|auditor|existing|stale|기존|이전|부모|조상|선행|원본)[^\n]{0,15}?(?:의\s*)?(?:tester|reviewer|auditor|테스터)[^\n]{0,10}?(?:verification|verdict|review|approval|sign[- ]?offs?|PASS|clearance|승인|판정|검증|통과|결과|효력)[^\n]{0,15}?(?:을|를|은|는)?\s*(?:inherit\w*|받\w*|물려|승계|계승|답습|갈음|대신|유지|그대로|쓴\w*|사용|이어)"),
    ...
    ("ambiguous-lower-t-alt",
     ...
     r"postponed|deferred|미뤄|보류)[^\n]{0,10}?(?:정한다|정해|결정|decided|determined|classified|sorted|분류|정리)|(?:먼저|우선|first)[^\n]{0,15}?(?:작업|실행|착수|진입|work|start|run|proceed)[^\n]{0,20}?(?:급|T0|T1|T2|tier|등급|grade|band|분류|classification)[^\n]{0,15}?(?:뒤에|나중|이후|추후|사후|later|after|postponed|deferred|미뤄|보류|pending))|(?:start|begin|run|assign|dispatch|handle|send|route|enter|go|land|fall|drop|move|shift|place|put|set|slot|park|feed|throw|pass|delegate|push|offload|execute|process|perform|queu\w*|enqueu\w*|shelv\w*|kick\w*|launch\w*|fire|seed|inject|insert|means?\s+(?:to\s+)?(?:start|begin|run|assign|enter|go)|시작|착수|진입|투입|돌입)\s+(?:to\s+|into\s+|at\s+|on\s+|in\s+|for\s+|onto\s+)?(?:the\s+|a\s+|an\s+)?"
    ...
    ("core-class-lower-t",
     ...
     r"|주니어|초급|잡무|청소|보조|부수|저렴|싼|하급|가벼운|경량|vendor|freelance|crew|external|outside|third[- ]?party|consult\w*|helper|assistant|bargain|frugal|thrifty|discount|budget|low[- ]paid|cut[- ]?rate|seasonal|low[- ]?price|low[- ]?rate|work[- ]?queue|developer|programmer|coder|workforce|staffer|staffing|agency|offshore|nearshore|pooled|in[-\s]?house|operations|ops\b|back[- ]?office|support[- ]?desk|billing)|(?:is|are|be|remains?|stays?|counts?\s+as|qualif\w*\s+as|classified\s+as|"
