...
FILE_FORBIDDEN = [
    ...
    ("verdict-inherit-after-change",
     ...
     r"commits?|sha|version|replacement|successor|descendant|derived|downstream|tip|커밋|버전|리비전))|(?:new|revised|updated|patched|rebased|squashed|amended|cherry[- ]?picked|grafted|ported|descendant|derived|downstream|successor|follow[- ]?up|subsequent|새|새로운|수정된|리베이스된)[^\n]{0,5}?(?:head|revision|commit|sha|version|커밋|버전|리비전)[^\n]{0,25}?(?:inherit\w*|substitut\w*|replac\w*|reuse|re-?use|carr\w*|retain\w*|transfer\w*|grandfather\w*|rides?|keep\w*|holds?|stands?\s+on|borrow\w*|rests?\s+on|leans?\s+on|relies?\s+on|승계|계승|갈음|대신|대체|유지|그대로)[^\n]{0,15}?(?:under|from|of|의|으로|에)?\s*(?:the\s+|its\s+|their\s+|this\s+|that\s+)?(?:previous|prior|old|earlier|parent|ancestor|predecessor|grandparent|existing|"
    ...
    ("core-class-lower-t",
     ...
     r"하급|초급|주변|가벼운|경량|사무|심부름|vendor|freelance|crew|external|outside|third[- ]?party|consult\w*|helper|assistant|bargain|frugal|thrifty|discount|budget|low[- ]paid|cut[- ]?rate|staffing|agency|offshore|nearshore|pooled|in[-\s]?house|operations|ops\b|back[- ]?office|support[- ]?desk|billing)\b)"),
    ("lower-t-first-core",
    ...
    ("periph-core-no-reclassify",
     ...
     r"그대로)|(?:continues?|proceeds?|stays?|keeps?|remains?|follows?)[^\n]{0,10}?(?:through|in|on|within|along)[^\n]{0,15}?(?:its|their|the|ordinary|normal|usual|same|existing|opening|original|submitted|assigned|기존)[^\n]{0,20}?(?:lane|track|queue|path|channel|route|레인|경로)|(?:remains?|stays?|keeps?|continues?|ends?\s+up|becomes?|goes|labeled|tagged|treated|counted|filed|booked|logged|lives?|dies|sits|rests?|lands?|falls?)\s+(?:an?\s+|the\s+|as\s+an?\s+|in\s+(?:its|their|the)\s+)?(?:UI|peripheral|cosmetic|visual|display|view|presentation|output|read[-\s]?only|theming|surface|layout|screen|icon|spacing|styling|styled|decor\w*|ornament\w*|aesthetic|cli\b|command[-\s]?line|front[- ]?end|web)(?:[-\s](?:ticket|item|work|task|job|change|patch|issue|label|category|classif\w*|tier|grade|lane|bucket|bracket|bin|작업|항목))?)|(?:핵심|core|안전|가드)[^\n]{0,20}?(?:slip\w*|sneak\w*|leak\w*|creep\w*|touch\w*|end\w*|land\w*|enter\w*)[^\n]{0,15}?(?:into|in|to|onto|"
