...
FILE_FORBIDDEN = [
    ...
    r"(?:without|sans|with\s+no|no)\s+(?:any\s+|an\s+)?(?:upper\s+|hard\s+|firm\s+)?(?:bounds?|limit|cap|ceiling|restriction|constraint)|(?:may|might|can|could)\s+(?:continue|proceed|run|go|extend|stretch|drag|repeat|revisit)[^\n]{0,15}?(?:without|sans|with\s+no|no)\s+(?:any\s+|an\s+)?(?:upper\s+|hard\s+|firm\s+)?(?:bounds?|limit|cap|ceiling|restriction|constraint)|(?:is|are|be|remains?|stays?|seems?|appears?|counts?\s+as|becomes?|turns?|goes|gets?|stands?|남는다|유지|두고|둔다)\s+(?:unbounded|boundless|uncapped|limitless|unlimited|unrestricted|unconstrained|infinite|endless|무제한|무한|상한\s*없)|(?:stop|halt|end|finish|close|conclude|complete|wrap|terminate|cease|cap|limit|restrict|cut|curtail|trim|그만|종료|멈추|"
