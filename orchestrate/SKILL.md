...
- **tester는 구현과 다른 family**(다른 세션·다른 모델 계열)를 쓴다 — 타사 풀이 물리적으로
  소진됐을 때만 `spawn-worker` §2-4 의 조건부 동일 계열 예외를 따른다. 검증 급은 구현
  급 이상으로 배정한다. 스폰 절차는 §4와 동일하게 `spawn-worker`를 따른다.
- tester 브리프의 중심 질문은 §0-④에서 정한 검증 급소 그대로다 — tester가 임의로
...
- **같은 세션 자체검증 무효**: 구현한 세션이 스스로 완료를 검증하면 같은 사각지대를
  반복해서 놓친다. 검증은 다른 세션·다른 모델 계열이 맡는다(타사 불가 시 조건부 동일 계열
  예외 — `spawn-worker` §2-4) → §5.
