...
- **tester는 구현과 다른 family**(다른 세션·다른 모델 계열)를 쓴다 — 같은 계열 tester 는
  `spawn-worker` §2-4 의 조건부 동일 계열 검증(또는 타사 풀 소진 시 지연 검증) 조건을 전부
  채울 때만 쓴다. 검증 급은 구현
...
- **같은 세션 자체검증 무효**: 구현한 세션이 스스로 완료를 검증하면 같은 사각지대를
  반복해서 놓친다. 검증은 다른 세션·다른 모델 계열이 맡는다(동일 계열은 조건부
  — `spawn-worker` §2-4) → §5.
