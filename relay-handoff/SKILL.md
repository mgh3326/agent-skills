...
**타겟 네이밍 관례**: 스킬 타겟은 **agent 이름**(`herdr agent rename <pane> <이름>`)이지
탭 라벨이 아니다(탭 rename은 사람 눈용). 관례: 허브=`orch` / 운영자 대화 세션=`<도구>-<용도>`
(예 `kiro-mock`) / 캡틴=`<도메인>-captain` / 워커=`<이슈>-<역할>`. 이름이 좋아도 read 실판별은
생략 금지.
