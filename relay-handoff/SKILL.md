...
**`comms-contract: v1`** — 이 절이 통신 계약의 **정본이고 유일한 버전 출처**다.
v1 의 요지: **완료를 아는 쪽(답변자·워커)이 요청자 레인으로 통지를 보낸다.**
요청자가 파일을 폴링하는 방식이 아니다.

🔴 **모든 패킷·브리프는 이 줄을 인용한다** — 머리에 `comms-contract: v1` 한 줄.
발신·수신이 서로 다른 판을 들고 있으면 아무 일도 안 일어나고 **에러도 안 난다**
(2026-09-16 실사고: 자문 2건의 완료 통지가 0/2 도착. 원인은 스킬 최신성이 아니라
요청자와 응답자의 **완료 계약 불일치**였고, 양쪽이 "어느 판"인지 말할 어휘가 없었다.
지시를 명시하니 2/2 도착했다).
계약이 바뀌면 **번호를 올리고** 기존 잡은 시작 시점 번호로 끝낸다 — 잡 중간에 바꾸지 않는다.

🔴 **버전을 갖는 것은 이번이 처음이다.** 이전 판들은 번호가 없었고, 그래서 위 사고가 났다.
...
grok 의 대기 상태(`N queued, Enter to send now`)는 공유 화면에서 페이로드 귀속이 불가능하므로 착지 증거가 아니다 — "무언가 대기 중" 이라는 모호 신호로만 읽고, 착지 주장은 전사 마커로만 한다 (#406, 2026-09-18).
