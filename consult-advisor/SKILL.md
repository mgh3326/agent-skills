...
description: 판단이 어려운 문제(설계 분기, 검증 판정 불일치, 위험한 결정)를 강모델 자문으로 푸는 절차. 자문처는 세션 이름 또는 티어(판단급)로 지정 — 상존 세션이 없으면 자문 전용 pane 세션을 스폰. 트리거 - "자문 받아줘", "강모델에 물어봐", "fable/opus/sol에 확인해줘", 판단 막힘·tester 간 판정 불일치.
---
...
pane인 기본 경로에서는 답변자가 답변 파일 작성 직후 아래 통지를 반드시 실행한다:
...
- 주입·제출검증 = **relay-handoff §3**, 답변 회수 = **ask-session §3**(답변 파일 계약+`lane.event` 대기).
